...
export const getClothesFriends = async (token, page, size) => {
    try {
      const url = new URL(ipAddress + "/fashion/clothes/friends");
...
export const getClothesPublic = async (token, page, size) => {
    try {
      const url = new URL(ipAddress + "/fashion/clothes/public");
      if (page !== undefined) url.searchParams.append("page", page);
      if (size !== undefined) url.searchParams.append("size", size);
