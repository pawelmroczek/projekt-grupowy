...
const VisibiltySelector = ({ value, setValue }) => {
  ...
  const description = visibilityDescription;
 
  const PrivateIcon = visibilityImages[privateValue];
  const FriendIcon = visibilityImages[friendValue];
  const PublicIcon = visibilityImages[publicValue];
  ...
  return (
    <View className="my-2">
      ...
      <View className="flex-row justify-between items-center w-full px-6 py-4">
        ...
        >
          <PrivateIcon size={40} color="gray" />
        </TouchableOpacity>
