...
public record ClothesGet(int id, Integer category, String name, String type,
                         ...
                         boolean clean, Integer visible, int priority, String picture, String user, List<Integer> pictogramIds) {
    public ClothesGet(Clothes clothes) {
        this(clothes.getId(), clothes.getCategory(), clothes.getName(), clothes.getType(), clothes.getColor(),
                ...
                clothes.getPicture().getUrl(), clothes.getUser().getEmail(), clothes.getPictograms().stream().map(Pictograms::getId).toList());
    }
}
